<?php
    $sql_sua_sp = "SELECT * FROM san_pham WHERE id_sanpham='$_GET[idsanpham]'LIMIT 1";
...
<form method="POST" action="modules/quanlysp/xuly.php?idsanpham=<?php echo $row['id_sanpham']?>" enctype='multipart/form-data'>
  <tr>
    <td>Tên sản phẩm </td>
    <td><input size="85" type="text" value="<?php echo $row['tensanpham']?>" name='tensanpham' ></td>
  </tr>
  <tr>
    <td>Mã sản phẩm  </td>
    <td><input size="85" type="text" value="<?php echo $row['masp']?>" name='masp' ></td>
  </tr>
  <tr>
    <td>Giá sản phẩm</td>
  ...
  <tr>
    <td>Tóm tắt </td>
    <td><textarea rows="10" cols="70" width="100%" name="tomtat" style="resize:none"><?php echo $row['tomtat']?></textarea></td>
  </tr>
  <tr>
    <td>Nội dung </td>
